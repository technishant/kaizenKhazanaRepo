...
class Kaizen extends \yii\db\ActiveRecord {
	...
	public function rules() {
		return [
			[['name', 'type', 'problem_observed', 'action_taken', 'processarea', 'attachmenttype',
				'category', 'tangiblebenifits', 'intengiblebenifits'], 'required', 'when' => function ($model, $attribute) {
					$model->type == 0;
			...
			[['name', 'subject', 'description', 'processarea', 'mode', 'tangiblebenifits',
				'intengiblebenifits', 'attachmenttype', 'attachmentbefore', 'attachmentafter',
				'problem_observed', 'action_taken'], 'string'],
			[['category', 'type', 'postedby', 'recordstatus'], 'integer'],
			[['costsaving', 'attachmentprocessed'], 'number'],
			[['kzfilebefore', 'kzfileafter'], 'file', 'extensions' => 'jpg,jpeg,png,mp4,3gp,mov,m4v,pdf,ppt,pptx',
				'mimeTypes' => 'image/jpeg,image/jpg,image/x-png,image/pjpeg, image/png,video/mpeg,video/mp4,video/quicktime,video/x-quicktime,video/x-m4v,video/mov,video/3gpp,application/pdf, application/vnd.ms-powerpoint, application/vnd.openxmlformats-officedocument.presentationml.presentation',
			...
			[['implementationdate', 'mode', 'approvedby', 'attachmentprocessed', 'attachmentbefore',
				'attachmentafter'], 'safe'],
			[['company', 'currrentstage', 'suggestedby', 'approvedby', 'attachmentother',
				'implemented_by'], 'string', 'max' => 255]
		];
	}
